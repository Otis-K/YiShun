...
export type ValidationCode = 'DUPLICATE_NODE_ID' | 'DUPLICATE_EDGE_ID' | 'UNKNOWN_NODE_TYPE' | 'MISSING_SOURCE_NODE' | 'MISSING_TARGET_NODE' | 'MISSING_SOURCE_PORT' | 'MISSING_TARGET_PORT' | 'SELF_CONNECTION' | 'DUPLICATE_CONNECTION' | 'PORT_TYPE_MISMATCH' | 'PORT_CARDINALITY' | 'REQUIRED_INPUT_MISSING' | 'CYCLE_DETECTED' | 'NODE_CONFIGURATION_INVALID';
...
export type NodeExecutor<TData extends CanvasNodeData = CanvasNodeData> = (context: NodeExecutionContext<TData>) => Promise<Record<string, unknown>> | Record<string, unknown>;
