...
(async () => {
  ...
  try {
    ...
    assert.match(await window.locator('.brandText small').textContent(), /0\.5\.25/);
    ...
    const result = await frame.evaluate(async () => {
      ...
      const beforeDismiss = Boolean(picker && picker.textContent.includes('已生成参考图') && picker.textContent.includes('真实视频素材') && !picker.textContent.includes('不应出现的伪视频') && picker.textContent.includes('上传本地素材'));
      ...
      return {
        ...
        videoDraggable: !video.classList.contains('nodrag'), beforeDismiss, centralPreview, videoLibraryPreview, materialColumns,
    ...
    assert.deepEqual(result, { width: 720, height: 680, videoDraggable: true, beforeDismiss: true, centralPreview: true, videoLibraryPreview: true, materialColumns: 4, inputHandles: 1, outputHandles: 1, dismissed: true });
    console.log(JSON.stringify({ ok: true, version: '0.5.25', installedCanvas: result }));
  } finally {
