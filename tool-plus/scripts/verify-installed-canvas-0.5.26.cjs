...
(async () => {
  ...
  try {
    ...
    assert.match(await window.locator('.brandText small').textContent(), /0\.5\.26/);
    ...
    const result = await frame.evaluate(async () => {
      ...
      return {
        ...
        videoDraggable: !video.classList.contains('nodrag'), imageTabValid, videoTabValid, exportWorked: downloadClicks.some(item => item.download.includes('已生成参考图')), centralPreview, videoLibraryPreview, materialColumns,
    ...
    assert.deepEqual(result, { width: 720, height: 680, videoDraggable: true, imageTabValid: true, videoTabValid: true, exportWorked: true, centralPreview: true, videoLibraryPreview: true, materialColumns: 4, inputHandles: 1, outputHandles: 1, dismissed: true });
    console.log(JSON.stringify({ ok: true, version: '0.5.26', installedCanvas: result }));
  } finally {
