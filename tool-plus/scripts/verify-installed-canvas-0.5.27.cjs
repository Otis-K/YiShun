...
(async () => {
  ...
  try {
    ...
    assert.match(await window.locator('.brandText small').textContent(), /0\.5\.27/);
    ...
    assert.deepEqual(result, { width: 720, height: 642, videoDraggable: true, frameInitiallyUnselected: true, generationModeAbsent: true, controlsAligned: true, imageTabValid: true, videoTabValid: true, exportWorked: true, centralPreview: true, videoLibraryPreview: true, materialColumns: 4, inputHandles: 1, outputHandles: 1, dismissed: true });
    console.log(JSON.stringify({ ok: true, version: '0.5.27', installedCanvas: result }));
  } finally {
